...
public class MotoRepository : IMotoRepository
{
    ...
    public async Task<List<Moto>> SearchAsync(Guid? clienteId, string? placa, int skip, int take, CancellationToken ct = default)
    {
        ...
        {
            var norm = placa.ToUpper().Replace("-", "").Trim();
            q = q.Where(x => EF.Functions.ILike(x.Placa.Value, $"%{norm}%"));
        }

        return await q
            .OrderBy(x => x.Marca).ThenBy(x => x.Modelo).ThenBy(x => x.Placa.Value)
            .Skip(skip).Take(take)
            .ToListAsync(ct);
    }
    ...
    {
        var norm = placa.ToUpper().Replace("-", "").Trim();
        var q = _db.Motos.AsQueryable().Where(x => x.Placa.Value == norm);
